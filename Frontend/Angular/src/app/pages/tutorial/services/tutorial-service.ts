...
const API_ROOT_URL = environment.HOST_TUTORIAL_API + '/api/tutorial';
...
export class TutorialService {
    ...
    create(item: TutorialModel): Observable<any> {
        const httpHeaders = this.httpUtils.getHTTPHeaders();
        return this.http.post<any>(API_ROOT_URL + '/insert', item, { headers: httpHeaders });
    }

    update(item: TutorialModel): Observable<any> {
        const httpHeaders = this.httpUtils.getHTTPHeaders();
    ...
    getAllGio(): Observable<QueryResultsModel> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
		return this.http.get<any>(API_URL + `/controllergeneral/Get_Gio`, { headers: httpHeaders });
	}
}