...
import { BacDaoTaoDataSource } from '../model/data-sources/bac-dao-tao.datasource';
...
import { BacDaoTaoEditDialogComponent } from '../bac-dao-tao-edit/bac-dao-tao-edit.dialog.component';
import { LayoutUtilsService } from 'src/app/_metronic/core/utils/layout-utils.service';

@Component({
  selector: 'app-bac-dao-tao-list',
  standalone: true,
  providers: [BacDaoTaoService, LayoutUtilsService, TokenStorage, { provide: MatPaginatorIntl, useClass: MatPaginatorIntlCustom }],
  ...
  templateUrl: './bac-dao-tao-list.component.html',
})
export class BacDaoTaoTableListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  itemModel: any;
  dataSource: BacDaoTaoDataSource;
  dataResult: any[] = [];
  ...
  ngOnInit(): void {
    ...
    this.dataSource = new BacDaoTaoDataSource(this.BacDaoTaoService);
  ...
  Delete(item: any) {
    ...
    Swal.fire(successAlert).then((clicked) => {
      if (clicked.isConfirmed) {
        this.BacDaoTaoService.delete(item.RowId).subscribe((res) => {
          if (res && res.status == 1) {
            this.loadDataList();
