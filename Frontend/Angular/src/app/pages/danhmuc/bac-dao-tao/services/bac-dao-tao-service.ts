...
const API_ROOT_URL = environment.HOST_TUTORIAL_API + '/api/bacdaotao';
const API_URL = environment.HOST_JEEHR_API + '/' + environment.apiUrl;
@Injectable({
    providedIn: 'root'
})

export class BacDaoTaoService {
    ...
    create(item: BacDaoTaoModel): Observable<any> {
        const httpHeaders = this.httpUtils.getHTTPHeaders();
        return this.http.post<any>(API_ROOT_URL + '/insert', item, { headers: httpHeaders });
    }

    update(item: BacDaoTaoModel): Observable<any> {
        const httpHeaders = this.httpUtils.getHTTPHeaders();
