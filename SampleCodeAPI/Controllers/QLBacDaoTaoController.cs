...
namespace SampleCodeAPI.Controllers
{
    ...
    [ApiController]
    [ApiVersion("1.0")]
    public class QLBacDaoTaoController(IConfiguration configuration, IConnectionCache connectionCache, ILogger<SampleController> logger, IProducer producer, INotifyService notifyService, IBoxEvent boxEvent, MinioObject minioClient, IConnectionService connectionService) : ControllerBase
    {
        ...
        public async Task<object> GetList([FromQuery] QueryParams query)
        {
            ...
            try
            {
                ...
                var result = await BusQLBacDaoTao.GetList(query, connect);
                return result;
            }
            catch (Exception ex)
            {
