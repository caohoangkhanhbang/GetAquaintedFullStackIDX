...
namespace SampleCodeAPI.Business
{
    public class BusQLBacDaoTao
    {
        ...
        public static async Task<BaseModel<object>> Insert(BacDaoTaoModels data, string connect, UserJWT loginData)
        {
            ErrorModel error = new ErrorModel();
            BaseModel<object> model = new BaseModel<object>();
            ...
            if (!CheckTrungTen(connect, data.Code))
            {
                model.status = 0;
                model.error = new ErrorModel
                {
            ...
            using (DpsConnection cnn = new DpsConnection(connect))
            {
                ...
                val.Add("HinhThucDaoTao", data.HinhThucDaoTao);
                val.Add("SoThuTu", data.SoThuTu);
                val.Add("CreatedBy", loginData.customdata.jeeAccount.userID);
                val.Add("CreatedDate", DateTime.UtcNow);

                if (cnn.Insert(val, "DM_BacDaoTao") == 1)
                {
                    int id = Convert.ToInt32(cnn.ExecuteScalar("SELECT IDENT_CURRENT ('DM_BacDaoTao') AS Current_Identity;  ").ToString());
                    data.RowId = id;
        ...
        public static async Task<BaseModel<object>> Update(BacDaoTaoModels data, string connect, UserJWT loginData)
        {
            BaseModel<object> model = new BaseModel<object>();
            BaseModel<string> result_up = new BaseModel<string>();
            ErrorModel error = new ErrorModel();
            ...
            if (!CheckTrungTen(connect, data.Code, data.RowId.ToString()))
            {
                model.status = 0;
                model.error = new ErrorModel
                {
            ...
            using (DpsConnection cnn = new DpsConnection(connect))
            {
                ...
                val.Add("HinhThucDaoTao", data.HinhThucDaoTao);
                val.Add("SoThuTu", data.SoThuTu);
                val.Add("UpdatedBy", loginData.customdata.jeeAccount.userID);
                val.Add("UpdatedDate", DateTime.UtcNow);

                if (cnn.Update(val, new SqlConditions { { "RowId", data.RowId } }, "DM_BacDaoTao") == 1)
                {
                    model.status = 1;
                    model.error = new ErrorModel
                    {
        ...
        public static async Task<BaseModel<object>> Delete(long id, string connect, UserJWT loginData)
        {
            ...
            using (DpsConnection cnn = new DpsConnection(connect))
            {
                ...
                if (cnn.Update(val, new SqlConditions { { "RowId", id } }, "DM_BacDaoTao") == 1)
                {
                    model.status = 1;
                    model.error = new ErrorModel
                    {
        ...
        public static bool CheckTrungTen(string _ConnectionString, string name, string Id = "")
        {
            ...
            {
                SqlConditions conds = new SqlConditions();
                string sql = "select * from DM_BacDaoTao where Code = @Code  and IsDel=0";
                conds.Add("Code", name);
